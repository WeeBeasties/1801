...
A total of `r length(myData$COMPETENCY)` registrations have been completed as of `r format(Sys.Date(),format="%B %d, %Y")`. This represents the efforts of `r length(unique(myData$NAME))` faculty, and includes a total of `r length(unique(myData$COURSE))` different courses. These numbers are down from last semester as shown by Figure \@ref(fig:comparison).
...
total <- c(281,length(myData$COMPETENCY),123,length(unique(myData$NAME)),138,length(unique(myData$COURSE)))
...
As compared to Fall 2017, the total number of registrations was down by `r round(100*((281-173)/281),1)`%. Moreover, the number of faculty members registering to report assessment was strongly down (by `r round(100*((123-62)/123),1)`%). This primarily reflects the fact the the Ferris Non-Tenured Faculty Organization members have elected to not participate in assessment. Since those members registered but did not report data last semester, the actual amound of data that we collect this semester may still be similar to that of last semester. Finally, the number of courses reporting is also down this semester (by `r round(100*((138-107)/138),1)`%). This may reflect an actual difference in the number of General Education courses offered each semester. Or... it may just reflect a decreasing rate of partipation in this process.
